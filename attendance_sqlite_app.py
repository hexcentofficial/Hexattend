...
# ---------------------------
def df_to_excel_bytes(df: pd.DataFrame, sheet_name="Attendance"):
    bio = io.BytesIO()
...
def df_to_pdf_bytes(df: pd.DataFrame, title="Attendance Report"):
    bio = io.BytesIO()
    # Use SimpleDocTemplate and Table
    ...
    if df.empty:
        ...
        # limit columns for readability: choose important fields
        ...
        if depts:
            ...
            if courses:
                ...
                if students:
                    ...
                    st.dataframe(df_students[["student_id","name","roll"]], use_container_width=True)
                else:
        ...
        if not depts:
            ...
            if not courses:
                ...
                if not students:
                    ...
                    options = [f"{r['id']}|{r['name']} ({r['roll'] if r['roll'] else ''})" for r in students]
                    ...
                    if not df_show.empty:
                        ...
                        st.dataframe(df_show[['college','department','course','student_name','roll','date','status']], use_container_width=True)
...
# ---------------------------
    ...
    if df.empty:
        ...
        st.dataframe(df[['college','department','course','student_name','roll','date','status']], use_container_width=True)
                ...
                if students:
                    ...
                    if start_date > end_date:
                        ...
                        if st.button("Generate Weekly Excel Report"):
                            # get attendance for these students within dates
                            ...
                            if df_week.empty:
                                ...
                                summary = df_week.groupby(['student_name','roll','date','status']).size().unstack(fill_value=0)
                                # we want counts of Present per student and percentage
...
# ---------------------------
    ...
    if st.button("Initialize / Reset DB (WARNING: This will not delete existing DB file if exists)"):
        init_db()
        st.success("DB initialized (tables ensured).")
    if Path(DB_PATH).exists():
        st.write("DB size:", f"{Path(DB_PATH).stat().st_size/1024:.1f} KB")
    if st.button("Download entire DB file"):
        with open(DB_PATH,"rb") as f:
            st.download_button("Download DB", data=f, file_name="attendance.db", mime="application/octet-stream")
